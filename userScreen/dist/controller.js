        ...
        return course.studentsCourse
          .map(function (student) {
            if (idUser === student.id) {
              return (
                '\n                    <div class="cardUserCourse" onclick="HandleOpenCourseU(\'' +
                course.uid +
                "','" +
                course.nameCourse +
                "')\">\n                        <h2>" +
...
function userAttendance() {
  ...
  var _userIndex = _studentsCourse.findIndex(function (student) {
    return student.id === idUser;
  });
  ...
  for (var i = 0; i < datesArr.length; i++) {
    html +=
      ...
      datesArr[i] +
      "<br>Attendance: " +
      checkAtt(i, _studentsCourse[_userIndex].attendance) +
      "<br><br>\n    ";
  }
  ...
  var html2 =
    "<h2>Summary: You have attendant " +
    sum +
    " from " +
    _studentsCourse[_userIndex].attendance.length +
    " lessons until now</h2>";
  console.log(html);
...
function userGrades() {
  ...
  for (var i = 0; i < datesArr.length; i++) {
    html +=
      ...
      datesArr[i] +
      "<br>Attendance: " +
      checkGrade(i, _studentsCourse[_userIndex].grades) +
      "<br><br>\n    ";
  }
  ...
  var html2 =
    "<h2>Your grade average is: " +
    sum / _studentsCourse[_userIndex].grades.length +
    "</h2>";
  console.log(html);
  userInnerGrade.innerHTML =
    ' <div class="attLesson">' + html + "</div>" + html2;
}
